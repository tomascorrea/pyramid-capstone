...
from pyramid_capstone.context import ParameterContext, extract_path_parameters_from_pattern, validate_path_pattern
from pyramid_capstone.exceptions import ParameterConflictError, ParameterMissingError
from pyramid_capstone.inspection import ParameterInfo, inspect_function_signature
...
def test_extract_path_parameters_from_request(app_request):
    ...
    request = app_request(path="/users/123", method="GET")
...
def test_extract_query_parameters_from_request(app_request):
    ...
    request = app_request(path="/users?limit=10&offset=0", method="GET")
...
def test_extract_json_body_parameters(app_request):
    ...
    request = app_request(path="/users", method="POST", json={"name": "John", "email": "john@example.com"})
...
def test_parameter_precedence(app_request):
    ...
    request = app_request(
        path="/users/123?user_id=456", method="POST", json={"user_id": "789"}  # Path and query params  # Body param
    )
    # Simulate matchdict that would be set by Pyramid routing
    request.matchdict = {"user_id": "123"}
...
def test_query_precedence_over_body(app_request):
    ...
    request = app_request(path="/users?name=query_name", method="POST", json={"name": "body_name"})
...
def test_build_simple_arguments(app_request):
    ...
    request = app_request(path="/users/123?name=John", method="GET")
    # Simulate matchdict that would be set by Pyramid routing
    request.matchdict = {"user_id": "123"}
...
def test_build_arguments_with_defaults(app_request):
    ...
    request = app_request(path="/users?name=John", method="GET")
