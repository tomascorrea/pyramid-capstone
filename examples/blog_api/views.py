...
def update_user(request, user_id: int, full_name: Optional[str] = None, 
                bio: Optional[str] = None, is_active: Optional[bool] = None) -> User:
    """Update a user."""
...
@th_api.get('/posts')
def list_posts(request, status: Optional[str] = None, category_id: Optional[int] = None,
               author_id: Optional[int] = None, page: int = 1, per_page: int = 10) -> dict:
    """
    ...
    total = blog_store.get_posts_count(
        status=post_status,
        category_id=category_id,
        author_id=author_id
    )
...
@th_api.post('/posts')
def create_post(request, title: str, content: str, author_id: int,
...
def update_post(request, post_id: int, title: Optional[str] = None,
                content: Optional[str] = None, excerpt: Optional[str] = None,
                category_id: Optional[int] = None, status: Optional[str] = None) -> Post:
    """Update a blog post."""
...
def get_blog_stats(request) -> dict:
    ...
    return {
        'users': {
            'total': len(blog_store.users),
            'active': len([u for u in blog_store.users.values() if u.is_active])
        },
        'posts': {
        ...
        'categories': {
            'total': len(blog_store.categories)
        },
        'comments': {
            'total': len(blog_store.comments)
        }
    }


@th_api.get('/users/{user_id}/posts')
def get_user_posts(request, user_id: int, status: Optional[str] = None,
