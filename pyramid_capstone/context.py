...
import re
from typing import Dict, Any, Set, List as ListType, Optional
...
class ParameterContext:
    ...
    def extract_request_parameters(self, request: Request) -> Dict[str, Any]:
        ...
        if (request.content_type and 'application/json' in request.content_type) or (hasattr(request, 'body') and request.body):
            try:
        ...
        for param_name, param_info in signature.get_non_request_parameters().items():
            if param_name in available_params:
                ...
                converted_value = self._convert_parameter_value(
                    raw_value, param_info, param_name
                )
                ...
                raise ParameterMissingError(
                    f"Required parameter '{param_name}' is missing from request"
                )
    ...
    def _convert_parameter_value(
        ...
        if isinstance(raw_value, str):
            try:
                ...
                raise ValueError(
                    f"Cannot convert parameter '{param_name}' value '{raw_value}' "
